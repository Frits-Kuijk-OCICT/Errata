...
Angular genereert nog steeds typescript code met puntkomma's en Kassenaars voorbeelden bevatten ze ook konsekwent. Met de introductie van Automatic Semiolon Insertion in javascript (lang voor 2016) zijn ze puntkomma's overbodig, tenzij je bijvoorbeeld perse twee statements op een regel wilt.
...
| 39 | cd angular-app | code angular-app |
| 39 | ng serve --open | In het linker pane van VSC open NPM SCRIPTS - run "start ng serve". Het rechteronder pane meldt: Executing tsak: "npm run start". De applicatie wordt nu gereed gemaakt en op zeker moment verschijnt "Angular Live Development Server is listening on localhost:4200, open your browser on http://localhost:4200/". Ctrl + klik op de link opent een webbrowser  |
| 44 en vele andere | .angular-cli.json | angular.json |
| 52 | npm install bootstrap --save | De optie is default met ingang van angular versie >= 12. Bootstrap is op versie 5.2.0 |
...
| 63 | 6 Ga terug ... Ctrl+C | Start het npm script "start ng server" nog eens, kies (rechtsonder) voor "restart task" |
| 81 | title:string<br>name:string | Dit levert foutslangetjes op. Klaarblijkelijk wordt de initialisatie in ngOnInit niet gedetecteerd. Miscchien toch maar bij de constructor blijven. De ngOnInit wordt uitgevoerd net voordat de component zichtbaar wordt. |
...
| 89-90 | Interface gebruiken | overslaan |
